...
class NeckChooser(BackgroundLayer, KeyListener):
  ...
  def render(self, visibility, topMost):
    ...
    if self.theme == 0 or self.theme == 1:
      ...
      wfactor = currentNeck.widthf(pixelw = 187.000)
      currentNeck.transform.reset()
      currentNeck.transform.translate(w/1.31,h/2)
      currentNeck.transform.scale(-wfactor,wfactor)
      currentNeck.draw()

      wfactor = lastNeck1.widthf(pixelw = 62.000)
      lastNeck1.transform.reset()
      lastNeck1.transform.translate(w*0.05,h*0.420)
      lastNeck1.transform.scale(-wfactor,wfactor)
      lastNeck1.draw()

      wfactor = lastNeck.widthf(pixelw = 62.000)
      lastNeck.transform.reset()
      lastNeck.transform.translate(w*0.175,h*0.554)
      lastNeck.transform.scale(-wfactor,wfactor)
      lastNeck.draw()

      wfactor = currentNeck.widthf(pixelw = 62.000)
      currentNeck.transform.reset()
      currentNeck.transform.translate(w*0.296,h*0.415)
      currentNeck.transform.scale(-wfactor,wfactor)
      currentNeck.draw()

      wfactor = nextNeck.widthf(pixelw = 62.000)
      nextNeck.transform.reset()
      nextNeck.transform.translate(w*0.42,h*0.554)
      nextNeck.transform.scale(-wfactor,wfactor)
      nextNeck.draw()

      wfactor = nextNeck1.widthf(pixelw = 62.000)
      nextNeck1.transform.reset()
      nextNeck1.transform.translate(w*0.539,h*0.402)
      nextNeck1.transform.scale(-wfactor,wfactor)
      nextNeck1.draw()

      if self.menu.currentIndex == 0:
        self.neckSelect.transform.reset()
        self.neckSelect.transform.translate(w*0.296, h*0.423)
      ...
      wfactor = currentNeck.widthf(pixelw = 192.000)
      currentNeck.transform.reset()
      currentNeck.transform.translate(w/1.22,h/2)
      currentNeck.transform.scale(-wfactor,wfactor)
      currentNeck.draw()

      wfactor = lastNeck1.widthf(pixelw = 64.000)
      lastNeck1.transform.reset()
      lastNeck1.transform.translate(w*0.067,h*0.420)
      lastNeck1.transform.scale(-wfactor,wfactor)
      lastNeck1.draw()

      wfactor = lastNeck.widthf(pixelw = 64.000)
      lastNeck.transform.reset()
      lastNeck.transform.translate(w*0.187,h*0.420)
      lastNeck.transform.scale(-wfactor,wfactor)
      lastNeck.draw()

      wfactor = currentNeck.widthf(pixelw = 64.000)
      currentNeck.transform.reset()
      currentNeck.transform.translate(w*0.307,h*0.420)
      currentNeck.transform.scale(-wfactor,wfactor)
      currentNeck.draw()

      wfactor = nextNeck.widthf(pixelw = 64.000)
      nextNeck.transform.reset()
      nextNeck.transform.translate(w*0.427,h*0.420)
      nextNeck.transform.scale(-wfactor,wfactor)
      nextNeck.draw()

      wfactor = nextNeck1.widthf(pixelw = 64.000)
      nextNeck1.transform.reset()
      nextNeck1.transform.translate(w*0.547,h*0.420)
      nextNeck1.transform.scale(-wfactor,wfactor)
      nextNeck1.draw()
