...
class GuitarSceneClient(GuitarScene, SceneClient):
  def createClient(self, libraryName, songName, Players):
    ...
    for i,guitar in enumerate(self.guitars):
      if guitar.isDrum:
        self.totalNotes[i] = len([1 for time, event in self.song.track[i].getAllEvents() if isinstance(event, Note)])
      else:
        self.totalNotes[i] = len(set(time for time, event in self.song.track[i].getAllEvents() if isinstance(event, Note)))
      self.totalSingleNotes[i] = len([1 for Ntime, event in self.song.track[i].getAllEvents() if isinstance(event, Note)])
    ...
    if self.stageMode != 2 and self.songStageEnabled == 0 and not self.playerList[0].practiceMode: #still need to load stage(s)
      ...
      if self.stageMode == 0 and self.isRotation == 0:  #MFH: just display a random stage
        ...
        for name in allfiles:
          ...
          else:  #catching invalid files
            Log.debug("Invalid background file " + name + " -- file ignored. Use .png files.")
      ...
      ##End check number of Stage-backgrounds
        ...
        for name in allfiles:
          ...
          if os.path.splitext(name)[1].lower() == ".png":
            ...
            Log.warn("Invalid background file: " + name + " -- file ignored.  Use .png files.")
      ...
      if self.guitars[num].isBassGuitar:
        ...
        if self.TSoundEnabled > 0: #MFH's cleaned-up - Faaa Drum sound
          if self.guitars[num].lastFretWasT1:
            self.sfxChannel.play(self.engine.data.T1DrumSound)
          elif self.guitars[num].lastFretWasT2:
            self.sfxChannel.play(self.engine.data.T2DrumSound)
          elif self.guitars[num].lastFretWasT3:
            self.sfxChannel.play(self.engine.data.T3DrumSound)
          elif self.guitars[num].lastFretWasC:
            self.sfxChannel.play(self.engine.data.CDrumSound)
        else:
          self.sfxChannel.play(self.engine.data.screwUpSoundDrums)
      else:   #guitar
  ...
  def doPick(self, num):
    ...
    if self.guitars[num].isDrum and self.bassKickSoundEnabled:
      if self.guitars[num].lastFretWasBassDrum:
        self.sfxChannel.setVolume(self.screwUpVolume)
        self.sfxChannel.play(self.engine.data.bassDrumSound)
...
#-----------------------
  ...
  def activateSP(self, num): #QQstarS: Fix this function, add a element "num"
    if self.guitars[num].starPower >= 50 and self.guitars[num].starPowerActive == False: #QQstarS:Set [0] to [i]
      ...
      self.sfxChannel.play(self.engine.data.crowdSound)
      self.sfxChannel.play(self.engine.data.starActivateSound)
...
#      return True  	    	  
  ...
  def initializeStarScoringThresholds(self):
    #so, a 1.0x multiplier's score would be self.totalSingleNotes[playerNum]*50.0
    #a 2.0x multiplier's score would be (2.0)(self.totalSingleNotes[playerNum]*50.0)
    #and so on...
    ...
    #player1:
    playerNum = 0
    self.avg1xScore.append(self.totalSingleNotes[playerNum]*50.0)
    ...
    if self.starScoring == 1:     #GH style scoring
      ...
      self.starThresholdsP1.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
      self.starThresholdsP1.append( (1, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first threshold(s) to surpass
      self.starThresholdsP1.append( (1, 1, int(0.05*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 2, int(0.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 3, int(0.15*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 4, int(0.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 5, int(0.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 6, int(0.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 7, int(0.35*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 0, int(0.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 1, int(0.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 2, int(0.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 3, int(0.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 4, int(0.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 5, int(0.90*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 6, int(1.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 7, int(1.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 0, int(1.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 1, int(1.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 2, int(1.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 3, int(1.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 4, int(1.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 5, int(1.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 6, int(1.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 7, int(1.90*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 0, int(2.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 1, int(2.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 2, int(2.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 3, int(2.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 4, int(2.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 5, int(2.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 6, int(2.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 7, int(2.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (5, 0, int(2.80*baseScore), int(0.90*self.totalNotes[playerNum]) ) ) 
      self.starThresholdsP1.append( (6, 0, int(0.00*baseScore), int(1.00*self.totalNotes[playerNum]) ) ) 
  
        
    elif self.starScoring == 2:   #RB style scoring
      #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
      self.starThresholdsP1.append( (0, 1, int(0.03125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 2, int(0.0625*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 3, int(0.09375*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 4, int(0.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 5, int(0.15625*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 6, int(0.1875*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 7, int(0.21875*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 0, int(0.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 1, int(0.28125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 2, int(0.3125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 3, int(0.34375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 4, int(0.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 5, int(0.40625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 6, int(0.4375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (1, 7, int(0.46875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 0, int(0.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 1, int(0.5625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 2, int(0.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 3, int(0.6875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 4, int(0.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 5, int(0.8125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 6, int(0.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (2, 7, int(0.9375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 0, int(1.0*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 1, int(1.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 2, int(1.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 3, int(1.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 4, int(1.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 5, int(1.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 6, int(1.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (3, 7, int(1.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 0, int(2.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 1, int(2.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 2, int(2.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 3, int(2.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 4, int(2.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 5, int(2.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 6, int(2.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (4, 7, int(2.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
      self.starThresholdsP1.append( (5, 0, int(3.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) 
      if self.guitars[playerNum].isBassGuitar:
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP1.append( (6, 0, int(4.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) 
      elif self.guitars[playerNum].isDrum:
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP1.append( (6, 0, int(4.65*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) 
      else:   #guitar parts
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP1.append( (6, 0, int(5.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) 
  
    else:   #0 = FoF scoring
      #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
      self.starThresholdsP1.append( (1, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 1, int(0.00*baseScore), int(0.03125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 2, int(0.00*baseScore), int(0.0625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 3, int(0.00*baseScore), int(0.09375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 4, int(0.00*baseScore), int(0.125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 5, int(0.00*baseScore), int(0.15625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 6, int(0.00*baseScore), int(0.1875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (1, 7, int(0.00*baseScore), int(0.21875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 0, int(0.00*baseScore), int(0.25*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 1, int(0.00*baseScore), int(0.28125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 2, int(0.00*baseScore), int(0.3125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 3, int(0.00*baseScore), int(0.34375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 4, int(0.00*baseScore), int(0.375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 5, int(0.00*baseScore), int(0.40625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 6, int(0.00*baseScore), int(0.4375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (2, 7, int(0.00*baseScore), int(0.46875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 0, int(0.00*baseScore), int(0.50*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 1, int(0.00*baseScore), int(0.53125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 2, int(0.00*baseScore), int(0.5625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 3, int(0.00*baseScore), int(0.59375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 4, int(0.00*baseScore), int(0.625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 5, int(0.00*baseScore), int(0.65625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 6, int(0.00*baseScore), int(0.6875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (3, 7, int(0.00*baseScore), int(0.71875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 0, int(0.00*baseScore), int(0.75*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 1, int(0.00*baseScore), int(0.775*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 2, int(0.00*baseScore), int(0.80*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 3, int(0.00*baseScore), int(0.825*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 4, int(0.00*baseScore), int(0.85*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 5, int(0.00*baseScore), int(0.875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 6, int(0.00*baseScore), int(0.90*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (4, 7, int(0.00*baseScore), int(0.925*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (5, 0, int(0.00*baseScore), int(0.95*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
      self.starThresholdsP1.append( (6, 0, int(0.00*baseScore), int(1.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
    ...
    if self.numOfPlayers > 1:   #repeat above for second array of thresholds
      ...
      if self.starScoring == 1:     #GH style scoring
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
        self.starThresholdsP2.append( (1, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first threshold(s) to surpass
        self.starThresholdsP2.append( (1, 1, int(0.05*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 2, int(0.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 3, int(0.15*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 4, int(0.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 5, int(0.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 6, int(0.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 7, int(0.35*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 0, int(0.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 1, int(0.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 2, int(0.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 3, int(0.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 4, int(0.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 5, int(0.90*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 6, int(1.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 7, int(1.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 0, int(1.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 1, int(1.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 2, int(1.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 3, int(1.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 4, int(1.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 5, int(1.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 6, int(1.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 7, int(1.90*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 0, int(2.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 1, int(2.10*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 2, int(2.20*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 3, int(2.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 4, int(2.40*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 5, int(2.50*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 6, int(2.60*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 7, int(2.70*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (5, 0, int(2.80*baseScore), int(0.90*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (6, 0, int(0.00*baseScore), int(1.00*self.totalNotes[playerNum]) ) )
    
          
      elif self.starScoring == 2:   #RB style scoring
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
        self.starThresholdsP2.append( (0, 1, int(0.03125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 2, int(0.0625*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 3, int(0.09375*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 4, int(0.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 5, int(0.15625*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 6, int(0.1875*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 7, int(0.21875*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 0, int(0.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 1, int(0.28125*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 2, int(0.3125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 3, int(0.34375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 4, int(0.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 5, int(0.40625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 6, int(0.4375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (1, 7, int(0.46875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 0, int(0.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 1, int(0.5625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 2, int(0.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 3, int(0.6875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 4, int(0.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 5, int(0.8125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 6, int(0.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (2, 7, int(0.9375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 0, int(1.0*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 1, int(1.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 2, int(1.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 3, int(1.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 4, int(1.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 5, int(1.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 6, int(1.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (3, 7, int(1.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 0, int(2.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 1, int(2.125*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 2, int(2.25*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 3, int(2.375*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 4, int(2.5*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 5, int(2.625*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 6, int(2.75*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (4, 7, int(2.875*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        self.starThresholdsP2.append( (5, 0, int(3.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        if self.guitars[playerNum].isBassGuitar:
          #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
          self.starThresholdsP2.append( (6, 0, int(4.80*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        elif self.guitars[playerNum].isDrum:
          #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
          self.starThresholdsP2.append( (6, 0, int(4.65*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
        else:   #guitar parts
          #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
          self.starThresholdsP2.append( (6, 0, int(5.30*baseScore), int(0.00*self.totalNotes[playerNum]) ) )
    
      else:   #0 = FoF scoring
        #                           (<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (0, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - first always all 0's
        self.starThresholdsP2.append( (1, 0, int(0.00*baseScore), int(0.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 1, int(0.00*baseScore), int(0.03125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 2, int(0.00*baseScore), int(0.0625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 3, int(0.00*baseScore), int(0.09375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 4, int(0.00*baseScore), int(0.125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 5, int(0.00*baseScore), int(0.15625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 6, int(0.00*baseScore), int(0.1875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (1, 7, int(0.00*baseScore), int(0.21875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 0, int(0.00*baseScore), int(0.25*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 1, int(0.00*baseScore), int(0.28125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 2, int(0.00*baseScore), int(0.3125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 3, int(0.00*baseScore), int(0.34375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 4, int(0.00*baseScore), int(0.375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 5, int(0.00*baseScore), int(0.40625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 6, int(0.00*baseScore), int(0.4375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (2, 7, int(0.00*baseScore), int(0.46875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 0, int(0.00*baseScore), int(0.50*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 1, int(0.00*baseScore), int(0.53125*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 2, int(0.00*baseScore), int(0.5625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 3, int(0.00*baseScore), int(0.59375*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 4, int(0.00*baseScore), int(0.625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 5, int(0.00*baseScore), int(0.65625*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 6, int(0.00*baseScore), int(0.6875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (3, 7, int(0.00*baseScore), int(0.71875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 0, int(0.00*baseScore), int(0.75*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 1, int(0.00*baseScore), int(0.775*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 2, int(0.00*baseScore), int(0.80*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 3, int(0.00*baseScore), int(0.825*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 4, int(0.00*baseScore), int(0.85*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 5, int(0.00*baseScore), int(0.875*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 6, int(0.00*baseScore), int(0.90*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (4, 7, int(0.00*baseScore), int(0.925*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (5, 0, int(0.00*baseScore), int(0.95*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
        self.starThresholdsP2.append( (6, 0, int(0.00*baseScore), int(1.00*self.totalNotes[playerNum]) ) ) #(<full stars>, <partial stars>, <score threshold>, <number of notes hit threshold>) - threshold(s) to surpass
  ...
  def updateStarsWithSustain(self, playerNum, tempExtraScore):
    if (self.inGameStars == 2 or (self.inGameStars == 1 and self.theme == 2) ) and self.stars[playerNum] < 6:
      self.lastStars[playerNum] = self.stars[playerNum]
      ...
      while (self.starThresholdIndex[playerNum] < self.maxStarThresholdIndex[playerNum]) and ( (self.playerList[playerNum].score + tempExtraScore) >= self.nextScoreThreshold[playerNum]) and (self.playerList[playerNum].notesHit >= self.nextHitNotesThreshold[playerNum]):
        ...
        #if (self.stars[playerNum] < self.nextStar[playerNum]) or (self.partialStar[playerNum] < self.nextPartialStar[playerNum]):
        self.stars[playerNum] = self.nextStar[playerNum]
        self.partialStar[playerNum] = self.nextPartialStar[playerNum]
        self.getNextStarThresholds(playerNum)
  
      if self.stars[playerNum] != self.lastStars[playerNum] and self.engine.data.starDingSoundFound:  #new star gained!
  ...
  def updateStars(self, playerNum):
    if (self.inGameStars == 2 or (self.inGameStars == 1 and self.theme == 2) ) and self.stars[playerNum] < 6:
      self.lastStars[playerNum] = self.stars[playerNum]
...
#----------------------------------------------  
  def updateAvMult(self, playerNum):
    ...
    if (self.inGameStats == 2 or (self.inGameStats == 1 and self.theme == 2) ):
      ...
      self.hitAccuracy[playerNum] = (float(self.playerList[playerNum].notesHit) / float(self.totalNotes[playerNum]) ) * 100.0
      self.avMult[playerNum] = self.playerList[playerNum].score/(self.totalSingleNotes[playerNum]*50.0)
...
#-            self.stars[playerNum] = 5
#-          else:
#-            self.stars[playerNum] = 4
...
#-          self.stars[playerNum] = 0
#-          self.partialStar[playerNum] = 0
#-        if self.hitAccuracy[playerNum] == 100.0 and self.playerList[playerNum].notesHit == self.totalNotes[playerNum]:
#-          self.stars[playerNum] = 6
#-        
#-          
#-      elif self.starScoring == 2:   #RB style scoring
...
#-        self.engine.data.starDingSound.play()
  ...
  def render(self, visibility, topMost):  #QQstarS: Fix this function for mostly. And there are lots of change in this, I just show the main ones
    ...
    if self.song:
      ...
      D = decimal.Decimal
      TWOPLACES = D(10) ** -(self.numDecimalPlaces)       # same as Decimal('0.01')
    ...
    if self.stageMode != 2 and self.song:
      if self.isRotation == 0:
        #MFH - division->constant precalculated        
        #imgwidth = self.background.width1()
        #wfactor = 640.000/imgwidth
      ...
      #myfingershurt:
        ...
        #MFH - TODO - conditional, are we animating or rotating?
        ...
        #self.imgArr[self.arrNum].transform.scale(wfactor,-wfactor)
        self.imgArr[self.arrNum].transform.reset()
        self.imgArr[self.arrNum].transform.translate(w/2,h/2)
        ...
        #except Exception, e:
        #  thisIsAProblemFrame = True
        #  Log.warn("Problem rendering animated background index " + str(self.arrNum) + ": " + str(e))

    #render the note sheet just on top of the background:
    if self.lyricSheet != None and self.song:
      #lyricimgheight = self.lyricSheet.height1()
      #MFH - division->constant
      #imgwidth = self.lyricSheet.width1()
      #wfactor = 640.000/imgwidth
      self.lyricSheet.transform.reset()
      self.lyricSheet.transform.translate(w/2, h*0.935)   #last change: -0.015
      #self.lyricSheet.transform.scale(wfactor,-wfactor)
...
##bookmark
            ...
            if self.youRock == True:
              if self.rockTimer == 1:
                self.sfxChannel.setVolume(self.sfxVolume)
                self.sfxChannel.play(self.engine.data.rockSound)
            ...
            if self.failed:
              ...
              if self.failTimer == 1:
                self.sfxChannel.setVolume(self.sfxVolume)
                self.sfxChannel.play(self.engine.data.failSound)
...
#===================================================#
        ...
        if self.song:
          ...
          if ((self.inGameStats == 2 or (self.inGameStats == 1 and self.theme == 2)) and (not self.pause and not self.failed)) and ( (not self.pause and not self.failed) or self.hopoDebugDisp == 1 ):
            ...
            trimmedTotalNoteAcc = D(str(self.hitAccuracy[i])).quantize(TWOPLACES)
            text = str(self.playerList[i].notesHit) + "/" + str(self.totalNotes[i]) + ": " + str(trimmedTotalNoteAcc) + "%"
            ...
            trimmedAvMult = D(str(self.avMult[i])).quantize(TWOPLACES)
              ...
              if self.isKillAnalog[i]:
                ...
                if self.analogKillMode[i] == 2: #xbox mode:
                  ...
                  if self.actualWhammyVol[i] > 0.0:
                    ...
                    glColor3f(0.5, 0.5, 0.5)  #gry
                text = str(D(str(self.actualWhammyVol[i])).quantize(TWOPLACES))
          ...
          if self.dispAccuracy[i] and not self.pause and not self.failed:
            ...
            trimmedAccuracy = D(str(self.accuracy[i])).quantize(TWOPLACES)
                  ...
                  if self.guitars[i].canGuitarSolo:
                    if self.guitars[i].guitarSolo:
                      ...
                      if self.guitarSoloAccuracy[i] == 100.0: #fablaculp: soloDescs changed
                        ...
                        soloScoreMult = 100
                        self.sfxChannel.play(self.engine.data.crowdSound)    #liquid
                        ...
                        soloScoreMult = 0
                        self.sfxChannel.play(self.engine.data.failSound)    #liquid
                      soloBonusScore = soloScoreMult * self.guitars[i].currentGuitarSoloHitNotes
                      player.score += soloBonusScore
                      trimmedSoloNoteAcc = D(str(self.guitarSoloAccuracy[i])).quantize(TWOPLACES)
              ...
              if self.guitars[i].canGuitarSolo:
                if self.guitars[i].guitarSolo:
                  ...
                  if not self.pause and not self.failed:
                    ...
                    trimmedIntSoloNoteAcc = D(str(tempSoloAccuracy)).quantize(TWOPLACES)
