...
class Layer(object):  #A graphical stage layer that is used to render the rockmeter.
  ...
  def render(self, visibility):
    ...
    for i,player in enumerate(self.stage.scene.playerList):
      ...
      or (self.canrender == 2 and self.engine.config.get("coffee", "failingEnabled") == False)):
        self.engine.drawImage(self.drawing, scale = (self.scale[0], self.scale[1]), 
                          coord = (self.position[0] * w, self.position[1] * h), 
                          rot = self.angle, color = self.color, rect = self.rect, 
                          stretched = 0)
...
class FontLayer(object): #defines layers that are just font instead of images
  def __init__(self, stage, section, font):
    ...
    self.color = get("color",   str, "1.0,1.0,1.0,1.0").split(",")
  ...
  def render(self, visibility):
    ...
    for i in range(len(self.stage.scene.playerList)):
      ...
      glColor4f(float(self.color[0]),float(self.color[1]),float(self.color[2]),float(self.color[3]))
...
class RockEffect(Effect):
  def __init__(self, layer, options):
    ...
    self.color       = str(self.opt.get("color", '(1,1,1,1)')).split(";")
  ...
  def apply(self, i):
    ...
    self.layer.color = eval(self.color[int(math.ceil(currentRock*len(self.color))-1)])
...
class MultEffect(Effect):
  def __init__(self, layer, options):
    ...
    self.rect       = (0, 1, multiplier/float(self.opt.get("vertrect"))-1, multiplier/float(self.opt.get("vertrect")))
  def apply(self, i):
    multStreak = self.stage.scene.scoring[i].streak
    if multStreak >= 30:
      multiplier = 4
    ...
    self.angle       = float(self.opt.get("angle", 0.0))
    self.rect       = (0.0, 1.0, float(multiplier-1)/float(self.opt.get("vertrect")), float(multiplier)/float(self.opt.get("vertrect")))
