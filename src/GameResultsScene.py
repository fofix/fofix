  ...
  def songLoaded(self, song):
    for i,player in enumerate(self.playerList):
      ...
      if notes:# ShiekOdaSandz: Determines the number of stars received at the end of the song; I modified Coffee's settings
        ...
        #MFH - so chords aren't counted as single notes when average multiplier / score is concerned:
        self.avMult = player.score/(singleNotes*50.0)

        #MFH - 3 star scoring styles  
        if self.starScoring == 1:     #GH style scoring
          if self.avMult >= 2.8:#ShiekOdaSandz: might add "must get 90%" if people don't like this; "and self.accuracy[i] = 90.0"
            #myfingershurt: yes, this is needed to ensure a proper 5* score.  If the accuracy isn't there, it's still a 4*:
            if self.accuracy[i] >= 90.0:
              self.stars[i] = 5
            else:
              self.stars[i] = 4
          elif self.avMult >= 2.0:#ShiekOdaSandz: changed so that you have higher threshold for 4 stars
            self.stars[i] = 4
          elif self.avMult >= 1.2:#ShiekOdaSandz: scorehero for 3 stars
            self.stars[i] = 3
          elif self.avMult >= .4:#ShiekOdaSandz: scorehero version
            self.stars[i] = 2
          elif self.avMult > 0:#ShiekOdaSandz: scorehero version
            self.stars[i] = 1
          elif self.avMult == 0:#ShiekOdaSandz: changed so that you cannot get 3 stars with a score = 0
            self.stars[i] = 0
          if self.accuracy[i] == 100.0 and player.notesHit == notes:
            self.stars[i] = 6
        elif self.starScoring == 2:   #RB style scoring

          if player.part.text == "Bass Guitar":
            if self.avMult >= 4.80:
              self.stars[i] = 6
            elif self.avMult >= 3.0:
              self.stars[i] = 5
            elif self.avMult >= 2.0:
              self.stars[i] = 4
            elif self.avMult >= 1.0:
              self.stars[i] = 3
            elif self.avMult >= 0.5:
              self.stars[i] = 2
            elif self.avMult > 0.25:
              self.stars[i] = 1
            else: 
              self.stars[i] = 0
          elif player.part.text == "Drums":
            if self.avMult >= 4.65:
              self.stars[i] = 6
            elif self.avMult >= 3.0:
              self.stars[i] = 5
          ...
          if self.accuracy[i] >= 95.0:#Blazingamer: 95% or more accuracy for 5 stars
            self.stars[i] = 5
          elif self.accuracy[i] >= 75.0:#Blazingamer: 75% or more accuracy for 4 stars
            self.stars[i] = 4
          elif self.accuracy[i] >= 50.0:#Blazingamer: 50% or more accuracy for 3 stars
            self.stars[i] = 3
          elif self.accuracy[i] >= 25.0:#Blazingamer: 25% or more accuracy for 2 stars
            self.stars[i] = 2
          elif self.accuracy[i] > 0:#Blazingamer: More than 0% accuracy for 1 stars
            self.stars[i] = 1
          elif self.accuracy[i] == 0:#Blazingamer: 0% accuracy for 0 stars
            self.stars[i] = 0
          if self.accuracy[i] == 100.0 and player.notesHit == notes:
            self.stars[i] = 6
