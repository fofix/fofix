...
if _updateTables > 0: #needs to handle old versions eventually.
    ...
    _playerDB.execute('CREATE TABLE `players` (`name` STRING UNIQUE, `lefty` INT, `drumflip` INT, `prodrums` INT, `autokick` INT, `assist` INT, `twochord` INT, `necktype` INT, `neck` STRING, \
...
def loadPlayers():
    ...
    for name in allplayers:
        ...
        if name.lower().endswith(".ini") and len(name) > 4:
            ...
            try:
                if len(pref) == 14:
                    playerpref.append((pref[1], pref[2], pref[3], pref[4], pref[5], pref[6], pref[7], pref[8], pref[9], pref[10], pref[11]))
            except TypeError:
                try:
                    ...
                    del c
                    _playerDB.execute('INSERT INTO `players` VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 1)', [playername[-1], lefty, drumf, pdrums, autok, assist, twoch, neckt, neck, part, diff, upname, control])
                    playerpref.append((lefty, drumf, pdrums, autok, assist, twoch, neckt, neck, part, diff, upname))
                except IOError:
                    _playerDB.execute('INSERT INTO `players` VALUES (?, 0, 0, 0, 0, 0, 0, 0, ``, 0, 2, ``, 0, 0, 1)', [playername[-1]])
...
def savePlayers():
    for pref in _playerDB.execute('SELECT * FROM `players` WHERE `changed` = 1').fetchall():
        try:
            ...
            c.set("player","prodrums",int(pref[3]))
            c.set("player","auto_kick",int(pref[4]))
            c.set("player","assist_mode",int(pref[5]))
            c.set("player","two_chord_max",int(pref[6]))
            c.set("player","necktype",int(pref[7]))
            c.set("player","neck",str(pref[8]))
            c.set("player","part",int(pref[9]))
            c.set("player","difficulty",int(pref[10]))
            c.set("player","name",str(pref[11]))
            c.set("player","controller",int(pref[12]))
            del c
...
def updatePlayer(player, pref):
    ...
    try:
        ...
        a = None
    if a is not None:
        _playerDB.execute('UPDATE `players` SET `name` = ?, `lefty` = ?, `drumflip` = ?, `prodrums` = ?, `autokick` = ?, `assist` = ?, `twochord` = ?, `necktype` = ?, `neck` = ?, \
